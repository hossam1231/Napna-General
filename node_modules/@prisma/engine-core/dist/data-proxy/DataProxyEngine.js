...
class DataProxyEngine extends import_Engine.Engine {
  ...
  request(query, headers = {}, transaction) {
    this.logEmitter.emit("query", { query });
    return this.requestInternal({ query, variables: {} }, headers, transaction);
  }
    ...
    return this.withRetry({
      ...
      callback: async ({ logHttpCall }) => {
        ...
        const response = await (0, import_request.request)(url, {
          method: "POST",
          headers: { ...(0, import_runtimeHeadersToHttpHeaders.runtimeHeadersToHttpHeaders)(headers), ...this.headers },
        ...
        if (data.errors) {
          if (data.errors.length === 1) {
            ...
            throw new import_PrismaClientUnknownRequestError.PrismaClientUnknownRequestError(data.errors, this.config.clientVersion);
          }
        }
        return data;
      }
    ...
    for (let attempt = 0; ; attempt++) {
      ...
      try {
        ...
        this.logEmitter.emit("error", {
          message: `Error while ${args.actionGerund}: ${(_a = e.message) != null ? _a : "(unknown)"}`
        });

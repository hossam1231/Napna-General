...
async function getConfig(options) {
  ...
  const data = await (0, import_ts_pattern.match)(cliEngineBinaryType).when(
    () => true,
    () => {
      return getConfigWasm(options);
    }
  ).with(import_fetch_engine.BinaryType.libqueryEngine, () => {
...
async function getConfigWasm(options) {
...
__name(getConfigWasm, "getConfigWasm");
...
function parseConfigError({ errorOutput, reason }) {
  const actualError = (0, import_function.pipe)(
    E.tryCatch(
      () => JSON.parse(errorOutput),
      () => {
    ...
    E.map((errorOutputAsJSON) => {
      ...
      const getConfigErrorInit = (0, import_ts_pattern.match)(errorOutputAsJSON).with({ error_code: "P1012" }, (eJSON) => {
        return {
          reason: (0, import_queryEngineCommons.createSchemaValidationError)(reason),
...
__name(parseConfigError, "parseConfigError");

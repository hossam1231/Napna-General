...
const createCache = (cacheClient, options) => {
  ...
  const fields = (options === null || options === void 0 ? void 0 : options.fields) || DEFAULT_LATEST_FIELDS;
  const cache = async (key, input, options) => {
    ...
    try {
      ...
      if (result) {
        logger === null || logger === void 0 ? void 0 : logger.debug(`[Cache] HIT key '${cacheKey}'`);
        return result;
      }
    } catch (e) {
      logger === null || logger === void 0 ? void 0 : logger.error(`[Cache] Error GET '${cacheKey}': ${e.message}`);

      // If client implements a reconnect() function, try it now
      if (e instanceof _errors.CacheTimeoutError && client.disconnect) {
        logger === null || logger === void 0 ? void 0 : logger.error(`[Cache] Disconnecting current instance...`);
    ...
    try {
      ...
      logger === null || logger === void 0 ? void 0 : logger.debug(`[Cache] MISS '${cacheKey}', SET ${(0, _stringify.default)(data).length} bytes`);
      return serialize(data);
    } catch (e) {
      logger === null || logger === void 0 ? void 0 : logger.error(`[Cache] Error SET '${cacheKey}': ${e.message}`);
  ...
  const cacheFindMany = async (key, model, options = {}) => {
    ...
    try {
      ...
      if (e instanceof PrismaClientValidationError) {
        logger === null || logger === void 0 ? void 0 : logger.error(`[Cache] cacheFindMany error: model does not contain \`${fields.id}\` or \`${fields.updatedAt}\` fields`);
      } else {
        logger === null || logger === void 0 ? void 0 : logger.error(`[Cache] cacheFindMany error: ${e.message}`);
      }
    ...
    if (latest) {
      ...
      logger === null || logger === void 0 ? void 0 : logger.debug(`[Cache] cacheFindMany: No data to cache for key \`${key}\`, skipping`);
      return serialize(await model.findMany(conditions));
    }

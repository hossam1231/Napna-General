...
const addEnvVarTask = (name, value, comment) => {
  return {
    ...
    task: () => {
      var _context9;
      const envPath = _path.default.join(getPaths().base, '.env');
      const content = (0, _flat.default)(_context9 = [comment && `# ${comment}`, `${name}=${value}`, '']).call(_context9);
      let envFile = '';

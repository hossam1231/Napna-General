  ...
  toHaveCached(cacheClient, keyOrExpectedValue, expectedValue) {
    ...
    if (stringKey) {
      ...
      for (const [cachedKey, cachedValue] of (0, _entries.default)(cacheClient.storage)) {
        ...
        if ((_regexKey = regexKey) !== null && _regexKey !== void 0 && _regexKey.test(cachedKey)) {
          found = true;
          foundKVPair = {
            key: cachedKey,
...
const _checkValueForKey = (cacheClient, cacheKey, expectedValue) => {
  try {
    var _cacheClient$storage$;
    const cachedStringValue = (_cacheClient$storage$ = cacheClient.storage[cacheKey]) === null || _cacheClient$storage$ === void 0 ? void 0 : _cacheClient$storage$.value;

    // Check if its a jest asymmetric matcher i.e. objectContaining, arrayContaining
    const expectedValueOrMatcher = (expectedValue === null || expectedValue === void 0 ? void 0 : expectedValue.$$typeof) === (0, _for.default)('jest.asymmetricMatcher') ? expectedValue : JSON.parse((0, _stringify.default)(expectedValue)); // Because e.g. dates get converted to string, when cached
